...
### 下一步：GM-09 Boost Settlement Integration

目标：把 GM-08 产生的 `PENDING` fitness boost 真正应用到健身打卡收益里，并在结算成功后扣库存、改状态。

GM-09 应该交付：

- 读取已绑定或可绑定的 `PENDING` boost
- 计算 `User.coins`、`PunchRecord.assetAwarded`、`SeasonMemberStat.seasonIncome` 的加成
- 成功结算后扣减对应 boost 库存
- 将 boost `ItemUseRecord` 从 `PENDING` 改为 `SETTLED`
- 覆盖重复结算、撤销打卡和过期边界
