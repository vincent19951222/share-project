...
| GM-16 | Card Pool Tuning | 已完成 | 否 |
| GM-17 | Ops Dashboard | 已完成 | 管理员可见 |
...
可以把它理解成：先把补给站未来要用的内容写成一套稳定配置，后面 GM-02 到 GM-19 都来查这套配置，不要每个地方自己硬编码。
...
GM-06 把“牛马补给站”的抽奖入口从占位升级成第一版可用闭环：用户可以消耗抽奖券单抽或十连，十连不足 10 张券时可以用银子补齐，奖励会在同一事务里结算到银子或背包库存。
...
## GM-16: Card Pool Tuning

GM-16 做的是“把已经上线的抽奖奖池调到可运营状态”：它不是新增玩法，而是对 active reward pool 做概率、可用性和校验层收口，避免用户抽到当前没有使用入口的奖励。

### 已完成内容

1. active 奖池权重收口到 100

`content/gamification/reward-pool.ts` 的启用奖励池现在总权重为 100，并按 GM-16 分层模型固定：
...
这样后续看抽奖概率、直接银子期望和稀有奖励占比时，不需要再手动换算。

2. 禁止未开放奖励进入 live draw pool

`validateGamificationContent()` 现在会拒绝这些 active 奖励：

- active title reward，因为当前还没有称号 / 外观库存闭环
- grant disabled item 的奖励
- grant 当前没有使用入口或兑换入口的 item effect

这保证用户抽到的 active item 至少能在当前版本里被使用、兑换或明确展示。

3. 保留现有抽奖行为边界

GM-16 没有修改抽奖 API、数据库结构、抽奖记录格式或十连保底算法。它只调整内容配置和校验规则，并用测试确认：

- active pool 总权重为 100
- 直接银子期望为 8.75
- 十连保底仍从 utility / social / rare 中替换
- rare 道具仍能正常进入背包

4. 清理过期用户文案

`SupplyStation` 今日效果空状态移除了旧的 GM-08 阶段提示，避免用户误以为道具使用还处于未开放状态。

### GM-16 总结

GM-16 是一次运营化收口：它没有扩展系统能力，但把抽奖奖池从“功能可跑”调整到“当前版本可运营”。GM-01 到 GM-15 搭出的闭环现在有了更明确的 active pool guardrail，后续新增奖励时会先被内容校验拦一遍。

## GM-17: Ops Dashboard

GM-17 做的是“给管理员一个只读运营观察面板”：它不新增玩法、不改奖池、不修资产，而是把 GM-01 到 GM-16 已经沉淀的数据聚合成一张健康看板，帮助发布前和运营中判断经济、兑换、弱社交、企微发送是否有风险。

### 已完成内容

1. 新增团队级运营快照服务

新增 `lib/gamification/ops-dashboard.ts`，核心入口是：

- `buildGamificationOpsDashboard({ teamId, now, windowDays })`
...
- 资产一致性：用户券余额是否等于券流水汇总，用户银子是否能由打卡收益、抽奖金币、补券消耗解释
- 抽奖券囤积：用户持有 30 张以上抽奖券时进入观察
- 兑换处理：待处理兑换超过 2 天时标记风险
- 弱社交频率：同一发送人对同一接收人近窗口点名达到 3 次时进入观察
- 真实福利成本：近窗口真实福利产出达到阈值时提醒关注预算
- 企业微信发送：近窗口失败日志会直接标记风险

3. 新增管理员读取 API

新增 `GET /api/admin/gamification/ops-dashboard`。
...
接口不写数据库，不发送企业微信，不创建团队动态。

4. 接入 `/admin` 页面

`/admin` 现在会在赛季设置上方显示“运营观察”面板。
...
刷新按钮重新读取管理员 API，只更新观察快照。

5. 补充测试覆盖

新增：

- `__tests__/gamification-ops-dashboard.test.ts`
- `__tests__/gamification-ops-dashboard-api.test.ts`
- `__tests__/gamification-ops-dashboard-panel.test.tsx`

覆盖健康状态、风险状态、认证 / 权限、管理员成功读取、面板渲染和刷新行为。

### GM-17 总结

GM-17 把游戏化从“功能闭环”推进到“可观察”：管理员可以在不改配置、不动资产的前提下，看到券、银子、兑换、弱社交、真实福利和企微发送的当前风险。它为 GM-18 的运营配置和 GM-19 的异常处理留出了清晰边界。
...
GM-18 把游戏化配置从“只能读代码理解”推进到“管理员可观察”：运营时可以先看 GM-18 确认当前规则是什么，再看 GM-17 判断真实数据是否健康。它仍然保持只读边界，为后续配置发布和变更审计预留空间。
...
- `docs/superpowers/plans/2026-05-02-gm-16-card-pool-tuning.md`
- `docs/superpowers/specs/2026-05-02-gm-17-ops-dashboard-design.md`
- `docs/superpowers/plans/2026-05-02-gm-17-ops-dashboard.md`
...
- 2026-05-02: GM-17 完成，新增只读管理员运营观察面板、团队级游戏化运营快照、管理员读取 API、资产一致性 / 抽奖券囤积 / 兑换延迟 / 弱社交频率 / 真实福利成本 / 企业微信失败风险卡和对应测试覆盖；未新增数据库模型，未修改奖池、道具、兑换、抽奖或资产规则。
- 2026-05-02: GM-16 完成，active 抽奖奖池权重收口到 100，按 coin / utility / social / cosmetic / rare 分层模型校验，禁用当前未闭环的 title 奖励，并阻止 disabled item 或未开放 item effect 进入 live draw pool。
