...
- `主线内容能力`：在已完成的团队动态 / 文档中心第一版上，继续补齐周报、企业微信推送和内容扩展
...
- 当前状态：第一版已完成
  已落地喇叭入口、预览面板、`/dynamics` 页面、未读标记和独立数据模型
...
- 当前状态：MVP 已完成（2026-04-28）
  已接入 Report Center 管理员生成 / 发布、`WeeklyReportDraft` 草稿持久化、周报 API、发布后写入 Team Dynamics，并在 `/dynamics` 页面渲染周报卡片
