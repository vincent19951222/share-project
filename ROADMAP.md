...
#### 1. 团队动态
- 已完成第一版
- 已落地 Navbar 头像左侧喇叭入口、预览面板、`/dynamics` 页面、未读标记和独立数据模型
- 已接入赛季开始 / 结束、赛季冲刺达成、团队全勤、连签里程碑等高价值事件
- 定位为团队消息 / 播报中心，承接“今天正在发生什么”和“哪些内容值得沉淀”
- 和战报中心分工明确：团队动态偏现场和时间线，战报中心偏复盘和分析
- 和共享看板分工明确：团队动态负责沉淀事件和回看；共享看板继续负责便签和公告
- 后续扩展：
  咖啡摘要事件
  共享看板公告或重要笔记引用
  企业微信推送内容来源
...
#### 2. 文档中心
- 已完成第一版
- 已落地 `/docs` 页面、Profile 下拉入口，以及 changelog / 赛季规则 / 使用说明 / 常见问题四类内容结构
- 定位为产品说明型内容中心，承接 changelog、规则说明、帮助文档、FAQ 等内容
- 和团队动态分工明确：文档中心回答“这是什么、怎么用”；团队动态回答“最近发生了什么”
- 后续扩展：
  牛马补给站等后续玩法说明内容
  文档内容维护入口或后台编辑能力
...
#### 3. 手动生成周报
- 已完成时间：2026-04-28
- 已接入 Report Center 管理员生成 / 发布、`WeeklyReportDraft` 草稿持久化、周报 API、发布后写入 Team Dynamics
- 已在 `/dynamics` 页面渲染周报卡片，周报可以作为团队动态沉淀和后续推送来源
- 周报已覆盖本周打卡次数、本周全勤日、本周运动高光成员、本周咖啡王、团队低谷日和高峰日、赛季冲刺进度和团队总结文案
- 后续扩展：
  历史周报索引
  企业微信一键推送
  周报卡片在移动端和长内容场景下的展示打磨
...
- `主线内容能力`：团队动态 / 文档中心 / 手动周报已经完成第一版，下一步继续补齐企业微信推送和内容扩展
...
- 当前状态：第一版已完成（2026-04-28）
- 已接入企业微信群机器人 Webhook 出站推送
- 管理员发布本周战报后自动推送企业微信 markdown 周报
- 未配置 `WEWORK_WEBHOOK_URL` 时自动跳过推送，不影响周报发布
- 不做复杂 OAuth、成员绑定和回调命令
- 支持推送：
  手动生成的周报
  后续补齐每日轻量日结
  后续补齐关键里程碑事件
- 后续再考虑更完整的集成管理页、推送记录和多群配置
...
### 主线最高优先级
- 打磨手动周报发布后的回看链路：Report Center 生成 / 发布、Team Dynamics 回看、后续推送入口保持一致
- 为团队动态继续接入咖啡摘要、共享看板公告 / 重要笔记引用
- 接入企业微信群机器人 Webhook，先支持周报和关键里程碑一键推送
...
### 主线次优先级
- 为文档中心补内容扩展与维护能力，承接牛马补给站规则说明
- Team Dynamics polish：收口 `initialNextCursor` 的前端语义（移除未使用状态或补“加载更多”）、为列表刷新 / 全部标为已读失败补用户可见错误提示、将 page / panel 列表 limit 抽为命名常量
- 打磨咖啡打卡页和牛马日历的移动端体验
...
- 最后更新：2026-04-28
- 当前状态：P3 基础闭环、团队动态第一版、文档中心第一版和手动周报 MVP 已落地；下一阶段聚焦周报回看 / 推送链路、Team Dynamics polish、企业微信推送和“牛马补给站”游戏化更新
