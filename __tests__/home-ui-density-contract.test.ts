...
describe("home tab density contract", () => {
  ...
  it("lets the report scene scroll without clipping the report and coffee sections", () => {
    ...
    const coffeeReportSource = readFileSync("components/report-center/CoffeeReportPanel.tsx", "utf8");
    const reportCss = stripMediaBlocks(css.replace(/\/\*[\s\S]*?\*\//g, ""));

    const boardRule = extractRuleBody(css, ".report-board");
    ...
    expect(coffeeReportSource).not.toContain("xl:col-span-1");
    expect(reportCss.match(/\.report-board\s*\{/g) ?? []).toHaveLength(1);
    expect(boardRule).toMatch(/height:\s*100%/);
    ...
    expect(contentRule).toMatch(/min-height:\s*100%/);
    ...
    expect(contentRule).not.toMatch(/grid-template-rows/);
    expect(contentRule).toMatch(/padding-inline:\s*clamp\(7\.5rem,\s*10vw,\s*12\.5rem\)/);
    expect(metricsRule).toMatch(/display:\s*grid/);
    expect(metricsRule).toMatch(/grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\)/);
    ...
    expect(bodyGridRule).toMatch(/min-height:\s*18rem/);
    expect(bodyGridRule).toMatch(/grid-template-columns:\s*minmax\(0,\s*1fr\)/);
    expect(chartFrameRule).toMatch(/flex:\s*1/);
